...
public class ExconProjectPersonsAuditRule implements DocumentAuditRule{
    ...
    public boolean processRunAuditBusinessRules(Document document) {
        ...
        valid &=projectHasTraveler(exconProjectDocument);
        
        return valid;
    }
    
    private boolean projectHasTraveler(ExconProjectDocument exconProjectDocument) { // move hardcoded values into a constants class
    	...
    	if (project!=null && StringUtils.equals("International Travel", project.getProjectType().getDescription())) {
    		if (project.getTraveler()==null) {
    			auditErrors.add(new AuditError("exconProjectPersonsBean.newPerson.personId", "error.exconProjectTraveler.missing",
    					"contacts.projectPersons"));
    			GlobalVariables.getAuditErrorMap().put("personsAuditErrors", new AuditCluster("Project Persons",auditErrors,Constants.AUDIT_ERRORS));
    			return false;
    		}
    	}
    	return true;
